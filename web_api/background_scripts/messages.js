    ...
    const onMessageListener = (preferences, request, sender, sendResponse) => {
        ...
        if (label === "toggleBlocking") {
            ...
            if (action === "block") {
                ...
                console.log(defaultStdIds)
                numBlockedStandardsForHost = defaultStdIds;
            } else if (action === "allow") {
                preferences.upcertRuleStandardIds(hostName, []);
                numBlockedStandardsForHost = [];
            }
        ...
        if (label === "blockedFeaturesReport") {
            if (!data || data.tabId === undefined) {
                ...
                //const reportData = {"data":{"0":{"url":"https://github.com/snyderp/web-api-manager/tree/master/sources","standards":{"111111"}}},"id":2117687311};
